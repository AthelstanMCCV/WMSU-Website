    ...
    <x-admin-head>
        <div class="p-4 sm:ml-64">
            <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 mt-14">
                @if($activeSections->isEmpty() && $archivedSections->isEmpty())
                ...
                @foreach($activeSections as $indicator => $group)
                <h2 class="text-4xl inter-extrabold mt-4 mb-4">{{ $indicator }}</h2>
            
                @if($indicator === 'Research & Extension News')
                    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                        <h3 class="text-2xl inter-bold mt-4 mb-4">Active News</h3>
                                ...
                                @foreach($group->groupBy('alt') as $alt => $newsGroup)
                                    <tr class="bg-white border-b border-gray-200">
                                        ...
                                        <td class="px-6 py-4 flex space-x-2">
                                            <button onclick="openEditNewsModal('{{ $alt }}')" class="px-3 py-1 bg-yellow-500 text-white rounded">Edit</button>
                                            <button type="button" onclick="openDeleteModal('{{ route('deleteNewsGroup', $alt) }}')" class="px-3 py-1 bg-red-500 text-white rounded">Archive</button>
                                        </td>
                                    </tr>
                                    <!-- Edit News Modal -->
                ...
                @endif
            @endforeach
            @foreach($archivedSections as $indicator => $group)
                @if($indicator === 'Research & Extension News')
                    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                        <h3 class="text-2xl inter-bold mt-4 mb-4">Archived News</h3>
                        <table class="w-full text-sm text-left rtl:text-right">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                                <tr>
                                    <th class="px-6 py-3">Title</th>
                            ...
                            <tbody>
                                @foreach($group->groupBy('alt') as $alt => $newsGroup)
                                    <tr class="bg-white border-b border-gray-200">
                                        ...
                                        <td class="px-6 py-4 flex space-x-2">
                                            <button onclick="openEditNewsModal('{{ $alt }}')" class="px-3 py-1 border border-white bg-yellow-500 hover:bg-white hover:text-yellow-500 hover:border-yellow-500 text-white rounded transition duration-300 ease-in-out">Edit</button>
                                            <button type="button" onclick="openRestoreModal('{{ route('restoreNewsGroup', $alt) }}')" class=" px-3 py-1 border border-white text-white bg-green-500 hover:bg-white hover:border hover:border-green-500 hover:text-green-500 rounded transition duration-300 ease-in-out">Restore</button>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
...
<div id="deleteModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center">
    <div class="bg-white p-6 rounded-lg w-[400px] relative">
        <h2 class="text-xl font-semibold mb-4 text-center">Confirm Archive</h2>
        <p class="mb-6 text-gray-700 text-center">Are you sure you want to archive this item?</p>
        ...
        <form id="deleteForm" method="POST">
            ...
            <div class="flex justify-center space-x-4">
                ...
                <button type="submit" class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700">Archive</button>
            </div>
        </form>
    </div>
</div>
...
<div id="restoreModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center">
    <div class="bg-white p-6 rounded-lg w-[400px] relative">
        ...
        <form id="restoreForm" method="POST">
            @csrf
            @method('PATCH') <!-- Restore is typically PATCH or POST -->
            <div class="flex justify-center space-x-4">
